...
class GoodsImgUpload
{
    ...
    */ 
    public function __construct($di, $allow_types = '') 
    {
        ...
        $this->max_size = $this->di['sysconfig']['allowedFileSize'];
    }
    ...
    */ 
    ...
    $originalHeight, $thumbWidth = 100, $thumbHeight = 100)
    {
        ...
        if($files['error'] == 0 && is_uploaded_file($files['tmp_name']))
        {
            ...
            //缩放比例后存储 
            if($this->resizeImage($files['tmp_name'], $this->original_max_width, $this->original_max_height, $this->originalName, '.jpg'))
            {
                if($this->resizeImage($this->save_path . $this->originalName . '.jpg',
                    $this->max_width, $this->max_height, $this->thumbName, '.jpg'))
                    return $this->save_re_path . $this->thumbName . '.jpg';
                else
                    return FALSE;
            }
            else
